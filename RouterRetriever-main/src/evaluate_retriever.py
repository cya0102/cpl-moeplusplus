...
class GateEvaluateRetrieval:
    ...
    def retrieve(self, corpus_embs, corpus_ids, corpus: Dict[str, Dict[str, str]], queries: Dict[str, str], **kwargs) -> Dict[str, Dict[str, float]]:
        if not self.retriever:
            raise ValueError("Model/Technique has not been provided!")
        return self.retriever.search(corpus_embs, corpus_ids, corpus, queries, self.top_k, self.score_function, **kwargs)
    ...
    def evaluate(qrels: Dict[str, Dict[str, int]], 
        ...
        for qid, cid_dict in qrels.items():
            if len(cid_dict) == 0:
                assert False
